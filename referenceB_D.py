...
class TopicSubscriber():
    ...
    def current_velocitycallback(self,msg):
        ...
        slope_c,intercept_c,_,_,_=stats.linregress((mat[0,0],((mat[0,0]-mat[3,0])/2)+mat[3,0],mat[3,0]),(mat[0,1],((mat[0,1]-mat[3,1])/2)+mat[3,1],mat[3,1]))

        
        if not self.points is None :
            
            #z_mask1=np.where(self.points[:,2]>=-1.6)
            #z_mask2=np.where(self.points[:,2]<=-0.5)
        
            #self.z_mask=z_mask1 and z_mask2
            self.z_mask1=np.where(self.points[:,2]<=-0.5)
            self.z_mask2=np.where(self.points[:,2]>=-1.5)
            self.z_mask=np.intersect1d(self.z_mask1,self.z_mask2)    
        
            #print(z_mask.shape,self.points.shape)
            p_zmasked=self.points[self.z_mask]
            self.p_zmasked=p_zmasked
                ...
                if len(self.vertices)>2:
                    ...
                    if len(self.vertices_voronoi)>3:
                        ...
                        if len(left_polygons)>2 and len(right_polygons)>2:
                            ...
                            #mask_a=np.where(mask_vor_L==False)
                            #mask_b=np.where(mask_vor_R==False)
                            #setxor=np.intersect1d(mask_a,mask_b)

                            #mask_ver=np.hstack((mask_a,mask_b))
                            #mask_vertices=np.unique(mask_ver)                        
...
def callback():
    ...
    Ts.velodyne_sub=rospy.Subscriber("/velodyne_points", senmsg.PointCloud2,Ts.pointcloudcallback)
    Ts.velocity_sub=rospy.Subscriber("/current_velocity", geomsg.TwistStamped,Ts.current_velocitycallback)
    rospy.spin()
    """
    ...
    while not rospy.is_shutdown():
        ...
        if not Ts.vertices_voronoi is None:
            vertices_X=Ts.vertices_voronoi[:,0]
            vertices_Y=Ts.vertices_voronoi[:,1]
            vertices_Z=-1.6
            ...
            for g in range(len(vertices_X)):
                p=geomsg.Point(vertices_X[g],vertices_Y[g],vertices_Z)
                marker.points.append(p)
            #marker.points=np.array(marker.points)
